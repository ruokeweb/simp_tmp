...
<script>
    ...
    export default {
        ...
        onLoad(event) {
            ...
            try {
                this.hasSuccess = JSON.parse(decodeURIComponent(hasSuccess));
            } catch (error) {
                this.hasSuccess = JSON.parse(hasSuccess);
            };
        ...
        onShareAppMessage(res) {
            //分享数据收集
            expand.saveShare(app.share_code.BACKSCHOOLRESULT_SHARE);
            var name = '';
            ...
            var truePath = encodeURIComponent(JSON.stringify('/pages/act/backSchoolDetail?id=' + encodeURIComponent(JSON.stringify(this.id))));
            return {
                title: name + '邀请你一起参加母校活动，赶快去报名吧',
                path: '/pages/tabbar/index/index?truePath='+truePath,
                imageUrl: '/static/img/act/act_default.png'
            }
        },
        methods: {
