...
layui.use(['form', 'layer', 'validparam', 'publicUtil', 'application'], function() {
    ...
    function formEdit(FormDatas) {
        if (FormDatas != '') {
            ...
            $(".password").val(publicUtil.htmlDecode(data.password));
            $(".url").val(publicUtil.htmlDecode(data.url));
            $(".port").val(publicUtil.htmlDecode(data.port));
    ...
    form.on("submit(addSchool)", function(data) {
        ...
        //异步请求
        $.ajax({
            url: application.SERVE_URL + "/sys/school/save", //ajax请求地址
            data: data,
