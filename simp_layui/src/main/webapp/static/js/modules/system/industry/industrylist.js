...
layui.use(['form', 'layer', 'laydate', 'table', 'laytpl', 'application', 'publicUtil'], function() {
	...
	function initTree() {
		_$.ajax({
			...
			success: function (data) {
				treeObj = $.fn.zTree.init($("#induTree"), setting, covert(data.data)); //加载数据
				//初始化
	...
	//右键点击事件
	table.on('rowRight(induList)', function(obj){
		publicUtil.show_menu(obj);
	});
	
	//左键点击事件
	table.on('row(induList)', function(obj){
	...
	function onClick(event, treeId, treeNode, clickFlag) {
		...
		treeCheckNode = treeNode.id;
			tableIns = table.render({
				elem: '#induList',
				//生产坏境下请求后台
				url : application.SERVE_URL+'/sys/industry/list',
	...
	_$(document).on('click', '.PER_ADD', function() {
		add(table.checkStatus('induList').data[0],"add");
	});

	//编辑操作
	_$(document).on('click', '.PER_EDIT', function() {
		var flag = publicUtil.jurgeSelectRows(table.checkStatus('induList').data);
		if(flag) {
			add(table.checkStatus('induList').data[0],"edit");
		} else {
			return false;
		}
	...
	_$(document).on('click', '.PER_DEL', function() {
		...
		if(flag) {
			layer.confirm('确定删除此数据吗？', {
				...
				_$.ajax({
					url: application.SERVE_URL + "/sys/industry/delete", //ajax请求地址
					data: {
						id: table.checkStatus('induList').data[0].id
					},
					success: function(data) {
	...
	function add(edit,action) {
		var restUrl=application.SERVE_URL + '/sys/industry/get';
		var id="edit"==action?(edit.id?edit.id:null):null;
		publicUtil.gotoEditPage(restUrl,id,"添加行业","industryadd.html")
	}
