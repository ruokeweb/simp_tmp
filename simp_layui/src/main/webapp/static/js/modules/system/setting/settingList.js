...
layui.use(['form','layer','laydate','table','laytpl','application','publicUtil'],function(){
    ...
    var tableIns = table.render({
		...
		,even : true 		
        ,elem: '#settingList'
        ,url : application.SERVE_URL+'/sys/setting/list',
        cellMinWidth : 95,
        page : true,
        height : "full-160",
        limit : 10,
	...
	$(document).on('click','.PER_EDIT',function(){		
		...
		if(flag){			
			_addDict(table.checkStatus('settingList').data[0]);
		}else{
			return false;
		}
	...
	$(document).on('click','.PER_DEL',function(){		
		...
		if(flag){
            layer.confirm('确定删除此此编码？',{icon:3, title:'提示信息'},function(index){				
				$.ajax({
					url: application.SERVE_URL+"/sys/setting/delete", //ajax请求地址
					data:{
	...
	function _addDict(edit){
		publicUtil.gotoEditPage(application.SERVE_URL +'/sys/setting/get',edit ==undefined?null:edit.id,"系统参数","settingAdd.html");
	}
