...
layui.use(['bodyTab', 'form', 'element', 'layer', 'jquery', 'application','publicUtil','dateUtils'], function () {
	var form = layui.form,
	...
	function initTopMenu(url) { //Ajax调用处理
		$.ajax({
			...
			success: function (res) {
				if(res.user.virtualName != null && res.user.virtualName != ""){
						$('.userName').html(res.user.virtualName);	
				}
				username = res.user.username;
				var photo= res.user.virtualPhoto;
				if(photo!=null&&photo!=""){
					document.getElementById("photo").src= application.SERVE_URL + application.FILEPATH + photo;	
					document.getElementById("userAvatar").src= application.SERVE_URL + application.FILEPATH + photo;
					// $(".user-photo a img").attr("src",application.SERVE_URL + photo);
				...
				var isUpdatePassword = JSON.parse(window.sessionStorage.getItem("isUpdatePassword"));

				if(isUpdatePassword.flag ){
					//layer.msg("需要修改密码");
					...
					var index = layui.layer.open({
						...
						shade: 0.8,
						area: ['800px', '65%'],
						content: '../../../../views/module/system/updatePwd/changePwd.html',
						success: function(layero, index) {
							top.layer.msg(isUpdatePassword.msg,{time:2000});
						},
