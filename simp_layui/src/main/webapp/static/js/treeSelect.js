				...
				$.ajax({
					...
					success: function(d) {
						...
						if (success) {
							...
							success(obj);
							var newinput = $('#' + TREE_SELECT_TITLE_ID + ' input');

							$('#' + TREE_SELECT_TITLE_ID + ' input').after("<div  class='laytreediv " + TREE_SELECT_TITLE_ID + "'></div>");
							$('#' + TREE_SELECT_TITLE_ID + ' input').remove();
							$("." + TREE_SELECT_TITLE_ID).append(newinput);

							var ids = $(elem).val();
							var treeObj = $('#' + TREE_SELECT_BODY_ID);
							var idArr = ids.split(",");
							var name = "";
							TREE_OBJ = $.fn.zTree.getZTreeObj(TREE_SELECT_BODY_ID);
							for (var i = 0; i < idArr.length; i++) {
								var treeObj = TREE_OBJ;
								//2.根据id查找节点
								...
								if (node != null) {
									...
									name += "<span class='laytreelabel'>" + node.name + '&nbsp;<i class="layui-icon layui-icon-close layclose"></i>' + "</span>";
								}

							}
							$('#' + TREE_SELECT_TITLE_ID + ' input').siblings().remove()
							$('#' + TREE_SELECT_TITLE_ID + ' input').before(name);
...
//				$('#' + TREE_SELECT_ID + ' li[treenode]').show();
			...
			onClick: function(event, treeId, treeNode) {
				...
				if (setting.check.enable) {
					...
					if (treeNode != null) {
						...
						TREE_OBJ.updateNode(treeNode);
						nodes = TREE_OBJ.getCheckedNodes(true);
						var v = "";
						var value = "";
						for (var i = 0; i < nodes.length; i++) {
							v += "<span class='laytreelabel'>" + nodes[i].name + '&nbsp;<i class="layui-icon layui-icon-close layclose"></i>' + "</span>";
						}
						$('#' + TREE_SELECT_TITLE_ID + ' input').siblings().remove()
					...
					var v = "<span class='laytreelabel'>" + name + '&nbsp;<i class="layui-icon layui-icon-close layclose"></i>' + "</span>";
					$('#' + TREE_SELECT_TITLE_ID + ' input').siblings().remove()
					$('#' + TREE_SELECT_TITLE_ID + ' input').before(v);
			...
			input: function() {
				...
				var selectHtml = '<div class="' + TREE_SELECT_CLASS + ' layui-unselect layui-form-select" id="' + TREE_SELECT_ID + '">' +
					...
					' <input type="text" id="' + TREE_INPUT_ID + '" placeholder="' + placeholder + '" value="" ' + readonly + ' class=" layui-unselect" style="border:0px;width:200px">' +
					'<i class="layui-edge"></i>' +
					'</div>' +
					'<div class="layui-anim layui-anim-upbit" style="">' +
					'<ul class="' + TREE_SELECT_BODY_CLASS + ' ztree" id="' + TREE_SELECT_BODY_ID + '"></ul>' +
					'</div>' +
					'</div>';
			...
			checkNodes: function(nodes) {
				for (var i = 0; i < nodes.length; i++) {
					...
					//$('#' + TREE_SELECT_ID + ' li[treenode]').show();
					//REE_OBJ.expandNode(node, true, false, true);
			...
			loadCss: function() {
				var ztree = '',
					ztree_ex = '.layui-treeSelect.layui-form-select .layui-anim{display:none;left:0;top:42px;padding:5px 0;z-index:9999;min-width:100%;border:1px solid #d2d2d2;max-height:300px;overflow-y:auto;background-color:#fff;border-radius:2px;box-shadow:0 2px 4px rgba(0,0,0,.12);box-sizing:border-box}.layui-treeSelect.layui-form-selected .layui-anim{display:block}',
					$head = $('head'),
		...
		$("body").on("click", '.layclose', function(e) {
			...
			for (var i = 0; i < nodes.length; i++) {
				if (nodes[i].name == name) {
					TREE_OBJ.selectNode(nodes[i]);
					nodes[i].checked = false;
					TREE_OBJ.updateNode(nodes[i]);
				}
			}
			nodes = TREE_OBJ.getCheckedNodes(true);
			if (check) {
