layui.define("form", function(exports) {
    ...
    interact.prototype.template = function(e) {
        ...
        if (e.selected) {
            ...
            o.each(hint, function(idx, value) {
                ...
                arr.push('<div class="layui-input-inline"><select  name="' + e.name +
                    '[]"><option value="">' + value + "</option>" + options + "</select></div>"
                )
            })
        }
    ...
    interact.prototype.select = function(e, obj, value) {
        ...
        if (value !== "") {
            var i = otl.index();
            if (!ot || !ot.length) {
                ot = otl.after('<div class="layui-input-inline"><select name="' + e.name +
                    '[]"><option value="">' + (hints[index] && hints[index][i] ? hints[index][i] :
                        "请选择") + "</option></select></div>").next()
            }
        ...
        o.each(e.selected, function(idx, value) {
            if (idx < e.selected.length - 1) {
                ...
                o.each(t.data(e.data)[value], function(index, item) {
                    options += '<option value="' + item.id + '" ' + (item.id == e.selected[
                        idx + 1] ? "selected" : "") + ">" + item.name + "</option>"
                });
                arr.push('<div class="layui-input-inline"><select name="' + e.name +
                    '[]"><option value="">' + (hint[idx] ? hint[idx] : "请选择") + "</option>" +
                    options + "</select></div>")
            }
        });
        return arr
    ...
    interact.prototype.refresh = function(e) {
        ...
        o(elems.join(",")).find(".layui-anim dd").click(function() {
            ...
            return filter ? layui.event.call(this, MOD_NAME, "interact(" + filter.attr("lay-filter") +
                ")", {
                    elem: obj.parents(".layui-input-inline").find("select option[value=" +
                        value + "]"),
                    othis: obj,
                    value: value,
                    text: obj.text()
