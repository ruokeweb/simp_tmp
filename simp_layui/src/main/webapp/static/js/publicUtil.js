...
layui.define(['form','layer','jquery','application','table','treeGrid'],function(exports){
	...
	//loading 
	var index = layer.load(2, {time: 5*1000,shade:0.1}); //又换了种风格，并且设定最长等待10秒 
	//关闭loading
	layer.close(index); 
            ...
            $.ajax({
				...
				success:function(result){
                   if(result.code==application.REQUEST_SUCCESS){
					   ...
					   var departCache=sessionStorage.getItem("departCache");
					   var departBaseCache=sessionStorage.getItem("departBaseCache");
                	   if(dict===null){
                	   ...
                	   if(departCache===null){
						   sessionStorage.setItem("departCache",JSON.stringify(result.data.departCache));
					   }
                       if(departBaseCache===null){
					   		sessionStorage.setItem("departBaseCache",JSON.stringify(result.data.departBaseCache));
					   }
                   }
				}
			})
...
// 					}
		...
		getObjFromStorageById: function(cacheName,id){
            if(id != null && id.length !=0 ){
                var obj = sessionStorage.getItem(cacheName);
		...
		errofunc :function(res){
			...
			if(result==undefined){
				...
				if(status===401){
					top.layer.msg("操作权限不足，或因为长时间未操作，请重新登录！(401)",{time:2000},function(){
						top.location.href = application.BASE_URL+"/login.html";
					});
				}else if(status===500){
					top.layer.msg("服务内部错误，请刷新页面或联系管理员 ！",{time:2000});
				}else{
					top.layer.msg("服务连接异常，请检查网络连接情况，并重新操作 ！",{time:2000});
				}

			}else{
				top.layer.msg(result.msg+"("+result.code+")",{time:2000});
			}
		},
		...
		//省市县3联
		selectAreaAndSetVal : function(parentId,selectid,selectValue){
			var area=sessionStorage.getItem("areaCache");
			// 抓取相关字段属性
		...
		//学校 学院 系 专业 
		selectDepartmentSetVal : function (parentId,selectid,selectValue){
			var department=sessionStorage.getItem("departCache");
			// 抓取相关字段属性
			var data=JSON.parse(department)[parentId];
			// $("#"+selectid).empty();
			if(data!=null){
		...
		update_check_ed: function (obj){
			...
			var pro = Object.keys(table.cache)[1];
			/*  tabCache 需要return*/
			if(pro!=undefined){
		...
		function judgeTokenIssue(){
			...
			//alert(application.TOKENTIME);
			if(application.TOKENISSUE>timelogin && timelogin>0){
				return true;
			}else if(timelogin < 0) {
				sessionStorage.removeItem("token");
				sessionStorage.removeItem("tokenTime");
				top.layer.msg("登录授权时间已截止，请重新登录!",{time:2000},function(){
					top.location.href = application.BASE_URL+"/login.html";
				});
				return false;
			}else{
